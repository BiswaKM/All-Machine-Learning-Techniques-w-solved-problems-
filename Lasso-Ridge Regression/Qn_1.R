comp <- read.csv('C:/Users/biswa/Desktop/360DigiTMG/8 . Module 8 - Lasso-Ridge Regression/Assignment/Datsets/50_Startups.csv') # loading the data 

attach(comp)

# Linear regression
m3 <- lm(Profit ~ .,data =comp )
summary(m3)

predprofit <- predict(m3, data=comp)
MSE3 <- mean(m3$residuals)^2


# Residual plot
plot(comp$Profit, predprofit)
...
comp_x <- model.matrix(Profit~.-1,data=comp)
comp_y <- comp$Profit
library(glmnet)
