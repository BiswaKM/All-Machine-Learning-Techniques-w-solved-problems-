comp <- read.csv('C:/Users/biswa/Desktop/360DigiTMG/8 . Module 8 - Lasso-Ridge Regression/Assignment/Datsets/Computer_Data.csv') # loading the data 
comp$X<-NULL
attach(comp)

# Linear regression
m3 <- lm(price ~ .,data =comp )
summary(m3)
...
comp_x <- model.matrix(price~.-1,data=comp)
comp_y <- comp$price
library(glmnet)
